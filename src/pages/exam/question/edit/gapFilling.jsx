...
const GapFilling = () => {
    ...
    const addItem = () => {
        setItems([...items, { prefix: (items.length + 1).toString(), content: '', score: '' }]);
    };
                ...
                <Form.Item label="填空答案">
                    {items.map((item, index) => (
                        <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
                            <span style={{ marginRight: 8, fontWeight: 'bold' }}>填空{item.prefix}：</span>
                            <Input value={item.content} onChange={(e) => updateItem(index, 'content', e.target.value)} style={{ marginRight: 8 }} placeholder="答案" />
                            <Input value={item.score} onChange={(e) => updateItem(index, 'score', e.target.value)} style={{ width: 100, marginRight: 8 }} placeholder="分数" />
                            <Button type="link" danger onClick={() => removeItem(index)}>删除</Button>
                        </div>
                    ))}
                    <Button type="dashed" onClick={addItem} style={{ width: '100%' }}>+ 添加填空</Button>
                </Form.Item>
